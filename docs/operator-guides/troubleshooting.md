...
| `npx skillstore-install ...` command copies but the URL looks wrong (e.g. `null` or `localhost` instead of your real domain) | The command is built client-side from `window.location.origin` — it reflects whatever host the browser thinks it's on                           | Make sure the site is accessed via its real public URL, not a proxied path that rewrites the origin                                                                                                                             |
| `npx skillstore-install ...` fails with "package not found" / npm 404                                                        | `skillstore-install` has not been published to the npm registry as of this writing                                                              | See [Deployment](./deployment.md#npx-install-requires-a-published-package) — this is a known precondition, not a per-site misconfiguration; publish `installer/` to npm yourself, or point visitors at the zip download instead |
...
- [Deployment](./deployment.md) — Docker and GitLab Pages setup this section assumes
